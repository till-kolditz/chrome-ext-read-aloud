...
/*
 * This code was downloaded from GitHub at
 * https://raw.githubusercontent.com/mozilla/readability/refs/heads/main/Readability.js
 * on December 16, 2025, commit ID 08be6b4bdb204dd333c9b7a0cfbc0e730b257252.
 */
...
function Readability(doc, options) {
  ...
  this._classesToPreserve =
      this.CLASSES_TO_PRESERVE.concat(options.classesToPreserve || []);
...
Readability.prototype = {
  ...
  // The default number of chars an article must have in order to return a
  // result
  ...
  _postProcessContent(articleContent) {
    // Readability cannot open relative uris so we convert them to absolute
    // uris.
  ...
  _fixRelativeUris(articleContent) {
    ...
    this._forEachNode(medias, function(media) {
      ...
      if (srcset) {
        var newSrcset =
            srcset.replace(this.REGEXPS.srcsetUrl, function(_, p1, p2, p3) {
    ...
    if (new RegExp(`\\s[${titleSeparators}]\\s`).test(curTitle)) {
      ...
      if (!match) {
        ...
        if (wordCount(curTitle) < 3) {
          ...
          // But if we have too many words before the colon there's something
          // weird with the titles and the H tags so let's just use the original
          // title instead
  ...
  _nextNode(node) {
    var next = node;
    while (next && next.nodeType != this.ELEMENT_NODE &&
  ...
  _prepArticle(articleContent) {
    ...
    // remove the top candidates even they have "share".
    ...
    // replace H1 with H2 as H1 should be only title that is displayed
    // separately
    this._replaceNodeTags(
        this._getAllNodesWithTag(articleContent, ['h1']), 'h2');

    // Remove extra paragraphs
    this._removeNodes(
        this._getAllNodesWithTag(articleContent, ['p']), function(paragraph) {
          // At this point, nasty iframes have been removed; only embedded video
          // ones remain.
          var contentElementCount = this._getAllNodesWithTag(
                                            paragraph,
                                            [
                                              'img',
                                              'embed',
    ...
    this._forEachNode(
        this._getAllNodesWithTag(articleContent, ['br']), function(br) {
    ...
    this._forEachNode(
        this._getAllNodesWithTag(articleContent, ['table']), function(table) {
  ...
  // works the way that it splits both texts into words and then finds words
  // that are unique in second text
  // the result is given by the lower length of unique parts
  _textSimilarity(textA, textB) {
    var tokensA =
        textA.toLowerCase().split(this.REGEXPS.tokenize).filter(Boolean);
    var tokensB =
        textB.toLowerCase().split(this.REGEXPS.tokenize).filter(Boolean);
  ...
  _grabArticle(page) {
    ...
    while (true) {
      ...
      while (node) {
        ...
        // User is not able to see elements applied with both "aria-modal =
        // true" and "role = dialog"
        ...
        if (!this._articleByline && !this._metadata.byline &&
          ...
          // Find child node matching [itemprop="name"] and use that if it
          // exists for a more accurate author name byline
        ...
        if (shouldRemoveTitleHeader && this._headerDuplicatesTitle(node)) {
          this.log(
              'Removing header: ', node.textContent.trim(),
        ...
        if (stripUnlikelyCandidates) {
          if (this.REGEXPS.unlikelyCandidates.test(matchString) &&
              ...
              !this._hasAncestorTag(node, 'code') && node.tagName !== 'BODY' &&
              node.tagName !== 'A') {
        ...
        if ((node.tagName === 'DIV' || node.tagName === 'SECTION' ||
             node.tagName === 'HEADER' || node.tagName === 'H1' ||
        ...
        if (node.tagName === 'DIV') {
          ...
          // Sites like http://mobile.slate.com encloses each paragraph with a
          // DIV element. DIVs with only a P element inside and no text content
          // can be safely converted into plain P elements to avoid confusing
          // the scoring algorithm with DIVs with are, in practice, paragraphs.
          if (this._hasSingleTagInsideElement(node, 'P') &&
      ...
      if (topCandidate === null || topCandidate.tagName === 'BODY') {
        ...
        // Move everything (not just elements, also text nodes etc.) into the
        // container so we even include text directly in the body:
        ...
        for (var i = 1; i < topCandidates.length; i++) {
          if (topCandidates[i].readability.contentScore /
        ...
        // higher scores than our topCandidate, but if we see the score going
        // *up* in the first few steps up the tree, that's a decent sign that
        // there might be more content lurking in other places that we want to
        // unify in. The sibling stuff below does some of that - but only if
        ...
        // help sibling joining logic when adjacent content is actually located
        // in parent's sibling node.
        parentOfTopCandidate = topCandidate.parentNode;
        while (parentOfTopCandidate.tagName != 'BODY' &&
      ...
      for (var s = 0, sl = siblings.length; s < sl; s++) {
        ...
        this.log(
            'Looking at sibling node:', sibling,
            sibling.readability ?
        ...
        if (append) {
          ...
          if (!this.ALTER_TO_DIV_EXCEPTIONS.includes(sibling.nodeName)) {
            // We have a node that isn't a common block level element, like a
      ...
      if (neededToCreateTopCandidate) {
        ...
        // siblings left for the previous loop, so there's no point trying to
        // create a new div, and then move all the children over. Just assign
        // IDs and class names here. No need to append because that already
        // happened anyway.
   ...
   * For now, only Schema.org objects of type Article or its subtypes are
   * supported.
      ...
      if (!metadata &&
        ...
        try {
          ...
          if (typeof parsed.name === 'string' &&
            ...
            // we have both name and headline element in the JSON-LD. They
            // should both be the same but some websites like aktualne.cz put
            // their own name into "name" and the article title to "headline"
            // which confuses Readability. So we try to check if either "name"
            // or "headline" closely matches the html title, and if so, use that
            // one. If not, then we use "name" by default.
  ...
  _getArticleMetadata(jsonld) {
    ...
    metadata.title = jsonld.title || values['dc:title'] ||
        values['dcterm:title'] || values['og:title'] ||
        values['weibo:article:title'] || values['weibo:webpage:title'] ||
    ...
    // get author
    metadata.byline = jsonld.byline || values['dc:creator'] ||
        values['dcterm:creator'] || values.author || values['parsely-author'] ||
        articleAuthor;

    // get description
    metadata.excerpt = jsonld.excerpt || values['dc:description'] ||
        values['dcterm:description'] || values['og:description'] ||
    ...
    metadata.publishedTime = jsonld.datePublished ||
        values['article:published_time'] || values['parsely-pub-date'] || null;
   ...
   *the <noscript> tag, and remove the <noscript> tag. This improves the quality
   *of the images we use on some sites (e.g. Medium).
   *
   * @param Element
   **/
  _unwrapNoscriptImages(doc) {
    // Find img without source or attributes that might contains image, and
    // remove it. This is done to prevent a placeholder img is replaced by img
    // from noscript in next step.
  ...
  _clean(e, tag) {
    ...
    this._removeNodes(this._getAllNodesWithTag(e, [tag]), function(element) {
      // Allow youtube and vimeo videos through as people usually want to see
      // those.
      if (isEmbed) {
   ...
   * @param  Function    filterFn a filter to invoke to determine whether this
   *     node 'counts'
   * @return Boolean
   */
  ...
  _markDataTables(root) {
    ...
    for (var i = 0; i < tables.length; i++) {
      ...
      // If the table has a descendant with any of these tags, consider a data
      // table:
  ...
  _fixLazyImages(root) {
    this._forEachNode(
        ...
        function(elem) {
          // In some sites (e.g. Kotaku), they put 1px square image as base64
          ...
          if (elem.src && this.REGEXPS.b64DataUrl.test(elem.src)) {
            ...
            // If it doesn't, then this src is important and shouldn't be
            // removed.
            ...
            // Here we assume if image is less than 100 bytes (or 133 after
          ...
          // also check for "null" to work around
          // https://github.com/jsdom/jsdom/issues/2580
  ...
  _cleanConditionally(e, tag) {
    ...
    this._removeNodes(this._getAllNodesWithTag(e, [tag]), function(node) {
      ...
      if (!isList) {
        ...
        this._forEachNode(
            listNodes, list => (listLength += this._getInnerText(list).length));
      ...
      if (this._getCharCount(node, ',') < 10) {
        ...
        if (this.REGEXPS.adWords.test(innerText) ||
        ...
        var textishTags =
            ['SPAN', 'LI', 'TD'].concat(Array.from(this.DIV_TO_P_ELEMS));
        ...
        const shouldRemoveNode = () => {
          ...
          if (!isList && !isFigureChild && headingDensity < 0.9 &&
            ...
            errs.push(`Suspiciously short. (headingDensity=${
                headingDensity}, img=${img}, linkDensity=${linkDensity})`);
          }
          if (!isList && weight < 25 &&
          ...
          if ((embedCount === 1 && contentLength < 75) || embedCount > 1) {
            errs.push(`Suspicious embed. (embedCount=${
                embedCount}, contentLength=${contentLength})`);
          }
  ...
  _isProbablyVisible(node) {
    // Have to null-check node.style and node.className.includes to deal with
    // SVG and MathML nodes.
    return (
        (!node.style || node.style.display != 'none') &&
        (!node.style || node.style.visibility != 'hidden') &&
        !node.hasAttribute('hidden') &&
        // check for "fallback-image" so that wikimedia math images are
        // displayed
  ...
  parse() {
    ...
    // If we haven't found an excerpt in the article's metadata, use the
    // article's first paragraph as the excerpt. This is used for displaying a
